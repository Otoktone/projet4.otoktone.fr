<p style="text-align: center">Il y a actuellement <?= $nombreNews ?> news.<br> En voici la liste :</p>
<div id="newsList">
    <table>
        <tr><th>Auteur</th><th>Titre</th><th>Date d'ajout</th><th>Dernière modification</th><th>Action</th></tr>
        <?php
        foreach ($listeNews as $news)
        {
            echo '<tr><td>', $news['auteur'], '</td><td>', $news['titre'], '</td><td>le ',
            $news['dateAjout']->format('d/m/Y à H\hi'), '</td><td>',
            ($news['dateAjout'] == $news['dateModif'] ? '-' : 'le '.$news['dateModif']->format('d/m/Y à H\hi')),
            '</td><td><a href="news-update-', $news['id'],
            '.html"><img src="/images/update.png" alt="Modifier" /></a> <a href="news-delete-',
            $news['id'], '.html"><img src="/images/delete.png" alt="Supprimer" /></a></td></tr>', "\n";
        }
        ?>
    </table>
</div>
<div id="report">
    <p style="color: darkred; text-align: center">Il y a <?= $nbReports?> commentaires signalés</p>
    <a style="padding-left: 10%;" href="reported.html">Gestion des commentaires</a>
</div>
